...
def test_system_check_merged_standard_merge(tmp_path: Path) -> None:
    ...
    with patch(
        "darkfactory.builtins.system_builtins.subprocess.run", side_effect=fake_run
    ):
...
def test_system_check_merged_not_merged(tmp_path: Path) -> None:
    ...
    with patch(
        "darkfactory.builtins.system_builtins.subprocess.run",
...
def test_system_check_merged_dry_run_no_subprocess(tmp_path: Path) -> None:
    ...
    with patch("darkfactory.builtins.system_builtins.subprocess.run") as mock_run:
        system_check_merged(ctx)
