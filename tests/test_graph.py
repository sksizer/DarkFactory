...
def test_build_graph_empty(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir, "PRD-001", "lone")
...
def test_build_graph_with_edges(tmp_prd_dir: Path) -> None:
    ...
    write_prd(tmp_prd_dir, "PRD-003", "third", depends_on=["PRD-001", "PRD-002"])
    g = graph.build_graph(load_all(tmp_prd_dir))
...
def test_detect_cycles_acyclic(tmp_prd_dir: Path) -> None:
    ...
    cycles = graph.detect_cycles(graph.build_graph(load_all(tmp_prd_dir)))
    assert cycles == []


def test_detect_cycles_self_loop(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir, "PRD-001", "a", depends_on=["PRD-001"])
    cycles = graph.detect_cycles(graph.build_graph(load_all(tmp_prd_dir)))
    assert len(cycles) == 1
    assert cycles[0] == ["PRD-001"]


def test_detect_cycles_two_node_cycle(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir, "PRD-001", "a", depends_on=["PRD-002"])
    write_prd(tmp_prd_dir, "PRD-002", "b", depends_on=["PRD-001"])
    cycles = graph.detect_cycles(graph.build_graph(load_all(tmp_prd_dir)))
    assert len(cycles) == 1
...
def test_topological_sort_linear(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir, "PRD-001", "a")
    write_prd(tmp_prd_dir, "PRD-002", "b", depends_on=["PRD-001"])
    write_prd(tmp_prd_dir, "PRD-003", "c", depends_on=["PRD-002"])
    order = graph.topological_sort(graph.build_graph(load_all(tmp_prd_dir)))
    assert order == ["PRD-001", "PRD-002", "PRD-003"]


def test_topological_sort_diamond(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir, "PRD-001", "a")
    write_prd(tmp_prd_dir, "PRD-002", "b", depends_on=["PRD-001"])
    write_prd(tmp_prd_dir, "PRD-003", "c", depends_on=["PRD-001"])
    write_prd(tmp_prd_dir, "PRD-004", "d", depends_on=["PRD-002", "PRD-003"])
    order = graph.topological_sort(graph.build_graph(load_all(tmp_prd_dir)))
...
def test_transitive_blocks(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir, "PRD-001", "a")
    write_prd(tmp_prd_dir, "PRD-002", "b", depends_on=["PRD-001"])
    write_prd(tmp_prd_dir, "PRD-003", "c", depends_on=["PRD-002"])
    write_prd(tmp_prd_dir, "PRD-004", "d", depends_on=["PRD-001"])
    g = graph.build_graph(load_all(tmp_prd_dir))
...
def test_is_actionable_ready_with_done_dep(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir, "PRD-001", "a", status="done")
    write_prd(tmp_prd_dir, "PRD-002", "b", status="ready", depends_on=["PRD-001"])
...
def test_is_actionable_ignores_draft(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir, "PRD-001", "a", status="draft")
    prds = load_all(tmp_prd_dir)
    assert not graph.is_actionable(prds["PRD-001"], prds)


def test_missing_deps(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir, "PRD-001", "a", depends_on=["PRD-999"])
    prds = load_all(tmp_prd_dir)
