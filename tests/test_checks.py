...
def test_empty_prds_returns_no_issues(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir / "prds", "PRD-001", "lone", status="ready")
...
def test_non_review_prds_ignored(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir / "prds", "PRD-001", "alpha", status="ready")
    write_prd(tmp_prd_dir / "prds", "PRD-002", "beta", status="done")
    write_prd(tmp_prd_dir / "prds", "PRD-003", "gamma", status="blocked")
    prds = load_all(tmp_prd_dir)
    git = _mock_git(set())
    assert validate_review_branches(prds, git) == []
...
def test_review_branch_present_returns_no_issues(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir / "prds", "PRD-001", "my-feature", status="review")
    prds = load_all(tmp_prd_dir)
...
def test_multiple_review_mixed_returns_correct_subset(tmp_prd_dir: Path) -> None:
    write_prd(tmp_prd_dir / "prds", "PRD-001", "alpha", status="review")
    write_prd(tmp_prd_dir / "prds", "PRD-002", "beta", status="review")
    write_prd(tmp_prd_dir / "prds", "PRD-003", "gamma", status="review")
    write_prd(tmp_prd_dir / "prds", "PRD-004", "delta", status="ready")
    prds = load_all(tmp_prd_dir)
