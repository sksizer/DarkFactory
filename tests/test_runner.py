...
def test_shell_cmd_is_format_stringed(tmp_path: Path) -> None:
    ...
    with patch("darkfactory.runner.run_shell_once") as mock_once:
...
def test_shell_failure_triggers_agent_retry(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch(
            "darkfactory.runner.run_shell_once", side_effect=shell_results
        ) as mock_shell,
    ):
        mock_invoke.return_value = InvokeResult(
            stdout="PRD_EXECUTE_OK: PRD-070\n",
...
def test_shell_failure_retry_also_fails(tmp_path: Path) -> None:
    ...
    with (
        patch("darkfactory.runner.invoke_claude") as mock_invoke,
        patch("darkfactory.runner.run_shell_once", return_value=failing) as mock_shell,
    ):
        mock_invoke.return_value = InvokeResult(
            stdout="PRD_EXECUTE_OK: PRD-070\n",
            stderr="",
