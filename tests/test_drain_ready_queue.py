...
class TestDiscoverReadyQueue:
    def test_empty_set(self, tmp_prd_dir: Path) -> None:
...
class TestMatchesFilters:
    ...
    def test_priority_filter_excludes_low(self, tmp_prd_dir: Path) -> None:
        high_prd = self._load_prd(tmp_prd_dir, "PRD-001", "high", priority="high")
        med_prd = self._load_prd(tmp_prd_dir, "PRD-002", "med", priority="medium")
        low_prd = self._load_prd(tmp_prd_dir, "PRD-003", "low", priority="low")
    ...
    def test_tag_filter_includes_match(self, tmp_prd_dir: Path) -> None:
        tagged = self._load_prd(tmp_prd_dir, "PRD-001", "tagged", tags=["harness"])
        untagged = self._load_prd(tmp_prd_dir, "PRD-002", "untagged", tags=[])
    ...
    def test_tag_or_semantics(self, tmp_prd_dir: Path) -> None:
        prd_a = self._load_prd(tmp_prd_dir, "PRD-001", "a", tags=["alpha"])
        prd_b = self._load_prd(tmp_prd_dir, "PRD-002", "b", tags=["beta"])
        prd_c = self._load_prd(tmp_prd_dir, "PRD-003", "c", tags=["other"])
    ...
    def test_exclude(self, tmp_prd_dir: Path) -> None:
        prd = self._load_prd(tmp_prd_dir, "PRD-005", "excluded")
        other = self._load_prd(tmp_prd_dir, "PRD-006", "other")
    ...
    def test_empty_filters_pass_all(self, tmp_prd_dir: Path) -> None:
        prd = self._load_prd(tmp_prd_dir, "PRD-001", "any", priority="low", tags=[])
...
class TestQueueExecution:
    ...
    def test_mid_run_unlock(self, tmp_prd_dir: Path) -> None:
