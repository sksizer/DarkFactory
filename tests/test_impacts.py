...
def test_effective_impacts_leaf_returns_declared(tmp_data_dir: Path) -> None:
    """A leaf PRD (no children) returns its own declared impacts."""
    write_prd(
        tmp_data_dir / "prds", "PRD-001", "a", impacts=["src/foo.rs", "src/bar.rs"]
    )
...
def test_effective_impacts_leaf_empty_returns_empty(tmp_data_dir: Path) -> None:
...
def test_effective_impacts_container_aggregates_children(tmp_data_dir: Path) -> None:
...
def test_effective_impacts_container_with_declared_raises(tmp_data_dir: Path) -> None:
...
def test_effective_impacts_nested_containers_aggregate(tmp_data_dir: Path) -> None:
    """Intermediate containers contribute via their descendants' leaves."""
    write_prd(tmp_data_dir / "prds", "PRD-001", "epic", kind="epic")
    write_prd(
        tmp_data_dir / "prds", "PRD-002", "feature", kind="feature", parent="PRD-001"
    )
    write_prd(
        tmp_data_dir / "prds",
        "PRD-003",
...
def test_effective_impacts_container_with_no_children_yet(tmp_data_dir: Path) -> None:
    ...
    write_prd(tmp_data_dir / "prds", "PRD-001", "unplanned-epic", kind="epic")
    prds = load_all(tmp_data_dir)
    assert impacts.effective_impacts(prds["PRD-001"], prds) == []


def test_effective_impacts_container_ignores_empty_leaves(tmp_data_dir: Path) -> None:
    """Leaves that don't declare impacts contribute nothing to the aggregate."""
    write_prd(tmp_data_dir / "prds", "PRD-001", "epic", kind="epic")
    write_prd(
        tmp_data_dir / "prds", "PRD-002", "declared", parent="PRD-001", impacts=["x.rs"]
    )
    write_prd(tmp_data_dir / "prds", "PRD-003", "undeclared", parent="PRD-001")  # empty
    prds = load_all(tmp_data_dir)
...
def test_impacts_overlap_disjoint(tmp_data_dir: Path) -> None:
    write_prd(tmp_data_dir / "prds", "PRD-001", "a", impacts=["src/foo.rs"])
    write_prd(tmp_data_dir / "prds", "PRD-002", "b", impacts=["src/bar.rs"])
    prds = load_all(tmp_data_dir)
    files = ["src/foo.rs", "src/bar.rs"]
...
def test_impacts_overlap_intersecting(tmp_data_dir: Path) -> None:
    write_prd(
        tmp_data_dir / "prds", "PRD-001", "a", impacts=["src/foo.rs", "src/bar.rs"]
    )
    write_prd(
        tmp_data_dir / "prds", "PRD-002", "b", impacts=["src/bar.rs", "src/baz.rs"]
    )
    prds = load_all(tmp_data_dir)
...
def test_impacts_overlap_glob_intersection(tmp_data_dir: Path) -> None:
    write_prd(tmp_data_dir / "prds", "PRD-001", "a", impacts=["src/*.rs"])
    write_prd(tmp_data_dir / "prds", "PRD-002", "b", impacts=["src/foo.rs"])
    prds = load_all(tmp_data_dir)
    files = ["src/foo.rs", "src/bar.rs"]
    overlap = impacts.impacts_overlap(prds["PRD-001"], prds["PRD-002"], files, prds)
...
def test_impacts_overlap_undeclared_returns_empty(tmp_data_dir: Path) -> None:
    write_prd(tmp_data_dir / "prds", "PRD-001", "a", impacts=["src/foo.rs"])
    write_prd(tmp_data_dir / "prds", "PRD-002", "b")  # no impacts declared
    prds = load_all(tmp_data_dir)
    files = ["src/foo.rs"]
    overlap = impacts.impacts_overlap(prds["PRD-001"], prds["PRD-002"], files, prds)
    assert overlap == set()
...
def test_impacts_overlap_parent_child_exempt(tmp_data_dir: Path) -> None:
...
def test_impacts_overlap_transitive_ancestor_exempt(tmp_data_dir: Path) -> None:
...
def test_impacts_overlap_siblings_still_warn(tmp_data_dir: Path) -> None:
...
def test_impacts_overlap_cross_tree_still_warns(tmp_data_dir: Path) -> None:
    """Unrelated PRDs touching the same file still overlap (no exemption)."""
    write_prd(tmp_data_dir / "prds", "PRD-001", "one", impacts=["src/foo.rs"])
    write_prd(tmp_data_dir / "prds", "PRD-002", "two", impacts=["src/foo.rs"])
    prds = load_all(tmp_data_dir)
    files = ["src/foo.rs"]
    overlap = impacts.impacts_overlap(prds["PRD-001"], prds["PRD-002"], files, prds)
    assert overlap == {"src/foo.rs"}
...
def test_impacts_overlap_epic_vs_unrelated_uses_aggregate(tmp_data_dir: Path) -> None:
...
def test_find_conflicts_excludes_parent(tmp_data_dir: Path) -> None:
    ...
    subprocess.run(["git", "init", "-q", "-b", "main"], cwd=tmp_data_dir, check=True)
    conflicts = impacts.find_conflicts(prds["PRD-001"], prds, tmp_data_dir)
