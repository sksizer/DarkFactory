"""Tests for system_runner.run_system_operation and its dispatch helpers."""
...
def _make_ctx(tmp_path: Path, *, dry_run: bool = True) -> SystemContext:
    op = SystemOperation(name="test-op", description="test", tasks=[])
...
def test_run_system_operation_empty_tasks(tmp_path: Path) -> None:
...
def test_run_builtin_unknown_name(tmp_path: Path) -> None:
    operation = _make_operation([BuiltIn("nonexistent_system_builtin")])
    ctx = _make_ctx(tmp_path)
    result = run_system_operation(operation, ctx)
...
def test_run_builtin_registered(tmp_path: Path) -> None:
    ...
    def my_builtin(ctx: SystemContext, **kwargs: object) -> None:
...
def test_run_builtin_raises_fails_step(tmp_path: Path) -> None:
    def bad_builtin(ctx: SystemContext, **kwargs: object) -> None:
...
def test_run_shell_format_string_substituted(tmp_path: Path) -> None:
    ...
    with patch("darkfactory.system_runner.run_shell_once") as mock_run:
        mock_proc = MagicMock()
...
def test_early_stop_on_failure(tmp_path: Path) -> None:
    ...
    def builtin_a(ctx: SystemContext, **kwargs: object) -> None:
        called.append("a")

    def builtin_b(ctx: SystemContext, **kwargs: object) -> None:
        called.append("b")
        raise RuntimeError("b failed")

    def builtin_c(ctx: SystemContext, **kwargs: object) -> None:
        called.append("c")
...
def test_pr_url_propagated(tmp_path: Path) -> None:
    def builtin_sets_pr(ctx: SystemContext, **kwargs: object) -> None:
