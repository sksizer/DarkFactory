...
1. **Turn `src/darkfactory/builtins.py` (582 lines, 9 public builtins) into a package.** Each public builtin gets its own submodule (`ensure_worktree.py`, `commit.py`, `create_pr.py`, …). Shared helpers live in `_shared.py`. The registry + `@builtin` decorator live in `_registry.py`. `__init__.py` re-exports the public API so every existing `from darkfactory.builtins import create_pr` keeps working.
...
- Helpers used by **2 or more** builtins → `src/darkfactory/builtins/_shared.py`.
...
- `@builtin(name)` decorator + `BUILTIN_REGISTRY: dict[str, BuiltInFunc]` live in `src/darkfactory/builtins/_registry.py`.
- `src/darkfactory/builtins/__init__.py` imports every submodule at the top so their `@builtin`-decorated functions register on package import. Order doesn't matter — the registry is just a dict.
...
- [ ] **AC-6** (post-549.3–549.11): Each of the nine builtins lives in its own submodule `src/darkfactory/builtins/<name>.py` with a colocated `<name>_test.py` that exercises the function's non-trivial branches.
- [ ] **AC-7** (post-549.11): Shared helpers used by more than one builtin live in `src/darkfactory/builtins/_shared.py`. Helpers used by exactly one builtin live in that builtin's own submodule.
- [ ] **AC-8** (post-549.12): The old `src/darkfactory/builtins.py` monolith and `tests/test_builtins.py` are both deleted. No dead code remains.
...
- Current `src/darkfactory/builtins.py` — 582 lines, 9 public builtins, 7 private helpers, registry decorator, all in one file.
