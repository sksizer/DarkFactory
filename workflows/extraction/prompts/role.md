...
(e.g. `~/Developer/darkfactory`), not on the pumice repo where you are
currently checked out.

The harness has created a pumice-side worktree for status tracking and
...
3. Verify against the PRD's Acceptance Criteria. Use whatever tools the
   ...
   no pumice-side `just test` for this work.
4. Stage and commit any pumice-side changes (typically just the PRD
   status update) with a conventional-commits message.

## You MUST NOT

- Modify pumice source code unless the PRD explicitly requires it.
- Push or open PRs in the target repo unless the PRD says so — the
  harness handles the pumice-side push/PR.
