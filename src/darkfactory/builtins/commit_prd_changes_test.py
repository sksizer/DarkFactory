...
def _init_git_repo(tmp_path: Path) -> None:
    ...
    sp.run(["git", "config", "user.email", "test@test.com"], cwd=str(tmp_path), check=True, capture_output=True)
    sp.run(["git", "config", "user.name", "Test"], cwd=str(tmp_path), check=True, capture_output=True)


def _setup_repo_with_prd(tmp_path: Path) -> tuple[Path, dict[str, object]]:
    ...
    return prd_path, prds  # type: ignore[return-value]


def test_no_changes_returns_cleanly(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _init_git_repo(tmp_path)
    prd_dir = tmp_path / "prds"
    prd_dir.mkdir()
...
def test_user_accepts_commit(tmp_path: Path) -> None:
    ...
    with patch("darkfactory.builtins.commit_prd_changes._prompt_user", return_value="y"):
        with patch("darkfactory.builtins.commit_prd_changes._git_diff_show"):
            commit_prd_changes(ctx)

    result = sp.run(["git", "log", "--oneline", "-1"], cwd=str(tmp_path), capture_output=True, text=True, check=True)
...
def test_user_skips_commit(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with patch("darkfactory.builtins.commit_prd_changes._prompt_user", return_value="n"):
        with patch("darkfactory.builtins.commit_prd_changes._git_diff_show"):
            commit_prd_changes(ctx)
...
def test_user_edits_message(tmp_path: Path) -> None:
    ...
    with patch("darkfactory.builtins.commit_prd_changes._prompt_user", side_effect=responses):
        with patch("darkfactory.builtins.commit_prd_changes._git_diff_show"):
            commit_prd_changes(ctx)
...
def test_other_dirty_files_noted(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    _, prds = _setup_repo_with_prd(tmp_path)
    ...
    result = sp.run(["git", "status", "--porcelain"], cwd=str(tmp_path), capture_output=True, text=True, check=True)
...
def test_commit_only_target_prd_file(tmp_path: Path) -> None:
    ...
    result = sp.run(["git", "diff", "--name-only", "HEAD~1", "HEAD"], cwd=str(tmp_path), capture_output=True, text=True, check=True)
