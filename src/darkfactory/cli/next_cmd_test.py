...
def test_cmd_next_no_actionable_prints_message(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_next_filters_by_capability(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_next_respects_limit(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_next_json_output(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_next_excludes_non_runnable(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cmd_next_json_empty(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    args = _make_args(tmp_path, json_output=True)
    with (
        patch("darkfactory.cli.next_cmd._load", return_value={}),
        patch("darkfactory.graph.is_actionable", return_value=False),
