...
<Tabs>
  <TabItem label="BuiltIn">
    ...
    BuiltIn(name="commit", kwargs={"message": "chore(prd): {prd_id} start work"})
    ```

    Common builtins: `ensure_worktree`, `set_status`, `commit`, `push_branch`, `create_pr`.
  </TabItem>
  <TabItem label="AgentTask">
    ...
    | `sentinel_success` | `"PRD_EXECUTE_OK"` | String agent must emit to signal success (followed by `:` and optional message) |
    | `sentinel_failure` | `"PRD_EXECUTE_FAILED"` | String agent emits to signal failure (followed by `:` and reason) |
  </TabItem>
...
sequenceDiagram
    ...
    R->>B: set_status(to="in-progress")
    R->>B: commit(start work)
...
2. `BuiltIn("set_status", kwargs={"to": "in-progress"})` -- transition [status](/concepts/status-lifecycle)
3. `BuiltIn("commit", kwargs={"message": "chore(prd): {prd_id} start work"})` -- checkpoint commit
4. `AgentTask("implement")` -- agent writes code
5. `ShellTask("test")` -- run test suite; retry agent on failure
6. `ShellTask("format")` -- run formatter; halt on failure
7. `ShellTask("lint")` -- run linter; retry agent on failure
8. `ShellTask("typecheck")` -- run type checker; retry agent on failure
9. `BuiltIn("set_status", kwargs={"to": "review"})` -- transition status
10. `BuiltIn("push_branch")` -- push branch to remote
11. `BuiltIn("create_pr")` -- create pull request
</Steps>
...
Workflow(
    ...
    applies_to=lambda prd, prds: prd.kind == "task",  # callable predicate
    priority=0,                          # higher = higher priority when multiple match
    tasks=[...],                         # ordered list of Task objects
    workflow_dir=Path(".darkfactory/workflows/default/"),
    template_name="prd-implementation",
)
```
