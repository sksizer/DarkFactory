...
Every [PRD](/concepts/prds) has a `status` field. Eight possible values, with transitions controlled by the harness, GitHub Actions, or manual editing.
...
| `superseded` | Replaced by a different PRD. Excluded from dependency graph. Terminal state eligible for archiving. |
...
stateDiagram-v2
    ...
    done --> superseded: Manual
    cancelled --> superseded: Manual
```

The happy path flows left to right: **draft** to **ready** to **in-progress** to **review** to **done**. The two recovery paths are **blocked** back to **ready** (when a human resolves the blocker) and **any state** to **cancelled** or **superseded** (manual abandonment or replacement).
...
<Tabs>
  <TabItem label="Automatic">
    ...
    **review -> done**
    Set by a GitHub Action on PR merge, or by `prd reconcile` which scans for merged PRs and updates statuses.
...
| `review` | `done` | Automatic: GitHub Action on PR merge, or `prd reconcile` |
...
| Any | `superseded` | Manual: when this PRD is replaced by another |
