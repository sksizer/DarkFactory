...
Run 1: Execute PRD-010 (planning workflow)
  → Planning agent writes PRD-011, PRD-012, PRD-013 to .darkfactory/data/prds/
...
Without re-loading between runs, child PRDs created by planning workflows would not appear in the graph until the next `prd run --execute` invocation. Re-loading enables single-invocation epic decomposition and execution.
...
prd run PRD-010 --execute --max-runs 3
```
